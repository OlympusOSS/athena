 ...
 */
async function revokeHydraLoginSessions(
	hydraAdminUrl: string,
	subject: string,
): Promise<void> {
	try {
		const res = await fetch(
			`${hydraAdminUrl}/admin/oauth2/auth/sessions/login?subject=${encodeURIComponent(subject)}`,
			{ method: "DELETE" },
		);
		...
		console.error(
			`Error revoking Hydra login sessions for ${subject}:`,
			err,
		);
	}
}
 ...
 * Non-throwing — logs errors but allows logout to proceed.
 */
async function revokeHydraConsentSessions(
	hydraAdminUrl: string,
	subject: string,
): Promise<void> {
	try {
		const res = await fetch(
			`${hydraAdminUrl}/admin/oauth2/auth/sessions/consent?subject=${encodeURIComponent(subject)}&all=true`,
			{ method: "DELETE" },
		);
		if (!res.ok && res.status !== 404 && res.status !== 204) {
			const text = await res.text().catch(() => "");
		...
		console.error(
			`Error revoking Hydra consent sessions for ${subject}:`,
			err,
		);
	}
}
...
export async function GET(request: NextRequest) {
	...
	const hydraPublicUrl =
		process.env.NEXT_PUBLIC_IAM_HYDRA_PUBLIC_URL || "http://localhost:4102";
	// IAM Hydra admin URL — needed to revoke login/consent sessions server-side
	const iamHydraAdminUrl =
		process.env.IAM_HYDRA_ADMIN_URL || "http://localhost:4103";
